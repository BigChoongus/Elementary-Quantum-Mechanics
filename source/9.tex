\chapter{A Mathematical Toolbox}
...
\begin{itemize}
\item $\text{Re}(z)$ is called the \sapos{real part} of the imaginary number $z$, which is, the real number $a$ if $z:=a+bi$.
\item $\text{Im}(z)$ is called the \sapos{imaginary part} of the imaginary number $z$, which is, the real number $b$ if $z:=a+bi$. Note that $\text{Im}(z)=b$ and not $\text{Im}(z)=bi$. 
\end{itemize}
...
\begin{enumerate}
    ...
    $$
    Here, we get the idea that the \sapos{middle two} values when we list the dimensions of the two matrices being multiplied together in the correct order must match up and then are discarded, leaving the number of rows of the left matrix and the number of columns of the right matrix as the dimensions of the resulting matrix after multiplication.
    \\\\
