\chapter{Time Evolution and Problem Solving Techniques}
...
\subsection{Observable Spaces}
When we refer to an \sapos{observable space}, we do not mean a space we can observe. We mean the state space when it is spanned by a physical observable. Precisely, that means working with the state space where the underlying basis is an eigenbasis of some observable operator.
\\\\
...
\subsubsection{Formation of Observable Operators}
\Answer
...
\begin{itemize}
    ...
    \item In the book we have referred to the state vector at times as \apos{storing information}, which is somewhat of a shorthand and can be confusing. What really is true is that the state vector is given meaning by the relation
    $$
